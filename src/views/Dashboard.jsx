    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex items-center group hover:shadow-md transition-all">
          ...
          <div>
            <p className="text-sm font-bold text-slate-500 uppercase tracking-wide">Meta del Mes</p>
            <p className="text-2xl font-black text-slate-800">$ {formatCurrency(stats.meta)}</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-2xl p-6 shadow-md flex items-center text-white transform hover:-translate-y-1 transition-transform">
          ...
          <div>
            <p className="text-sm font-bold text-emerald-50 uppercase tracking-wide">Colocación Actual</p>
            <p className="text-3xl font-black">$ {formatCurrency(stats.actual)}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex items-center group hover:shadow-md transition-all">
          ...
          <div>
            <p className="text-sm font-bold text-slate-500 uppercase tracking-wide">Brecha (Falta)</p>
            <p className="text-2xl font-black text-amber-600">$ {formatCurrency(stats.brecha)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
          ...
          <div className="relative pt-8 pb-4">
            ...
            <div className="flex justify-between mt-3 text-sm font-bold text-slate-400">
              <span>$0</span>
              <span className="text-slate-700">Meta: ${formatCurrency(stats.meta)}</span>
            </div>
          </div>
        </div>
                ...
                return (
                  <div key={index} className={`flex items-center justify-between p-4 rounded-xl ${bgClass}`}>
                    <div className="flex items-center">
                      <div className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center font-black text-slate-700 mr-4 border border-slate-100">
                        {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index + 1}`}
                      </div>
                      <div>
                        <p className="font-bold text-slate-800 text-sm uppercase">{asesor.nombre}</p>
                        <p className="text-xs font-semibold text-slate-500 flex items-center">{icon} <span className="ml-1">Top {index + 1}</span></p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-emerald-600 text-lg">${formatCurrency(asesor.venta)}</p>
                    </div>
                  </div>
                );
              })}
